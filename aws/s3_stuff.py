...
class S3_Stuff():    
    ...
    def upload_is_last_to_s3(s3, user_id, absolute_start_time, is_last):
        bucket_name = 's3-smart-alarm-app'
        absolute_timestamp = datetime.datetime.fromtimestamp(round(absolute_start_time))
        key = f"users/{user_id}/{absolute_timestamp.strftime('%Y%m%d_%H%M%S')}/is_last.json"

        json_data = json.dumps({"isLast": is_last}).encode('utf-8')

        try:
            s3.meta.client.put_object(Bucket=bucket_name, Key=key, Body=json_data)
            print(f"Uploaded is_last to s3://{bucket_name}/{key} directly from memory.")
        except ClientError as e:
            print(f"Couldn't upload is_last to {bucket_name}.")
            print(f"S3 Error: {e.response['Error']['Message']}")
        except Exception as e:
            print(f"Couldn't upload is_last to {bucket_name}.")
            print(f"Other Error: {e}")
